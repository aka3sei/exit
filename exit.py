...
with st.container():
    ...
    with col1:
        price_now = st.number_input("価格 (万円)", min_value=1, value=13752) 
...
if clicked:
    ...
    # ★ 修正箇所：ベース価格と表面利回りを同じ st.metric で大きく表示
    ...
    with col5_1: st.markdown(f'<div class="prediction-card border-light"><div class="pred-label">5年後 (年1%)</div><div class="pred-price">{round(p_5y[0]):,}</div><div class="pred-diff">現在比 {get_diff_text(p_5y[0], price_now)}</div></div>', unsafe_allow_html=True)
    with col5_2: st.markdown(f'<div class="prediction-card border-medium"><div class="pred-label">5年後 (年3%)</div><div class="pred-price">{round(p_5y[1]):,}</div><div class="pred-diff">現在比 {get_diff_text(p_5y[1], price_now)}</div></div>', unsafe_allow_html=True)
    with col5_3: st.markdown(f'<div class="prediction-card border-heavy"><div class="pred-label">5年後 (年5%)</div><div class="pred-price">{round(p_5y[2]):,}</div><div class="pred-diff">現在比 {get_diff_text(p_5y[2], price_now)}</div></div>', unsafe_allow_html=True)

    # 📅 10年後予測
    st.write("### 📅 10年後の市場シナリオ別予測")
    col10_1, col10_2, col10_3 = st.columns(3)
    with col10_1: st.markdown(f'<div class="prediction-card border-light"><div class="pred-label">10年後 (年1%)</div><div class="pred-price">{round(p_10y[0]):,}</div><div class="pred-diff">現在比 {get_diff_text(p_10y[0], price_now)}</div></div>', unsafe_allow_html=True)
    with col10_2: st.markdown(f'<div class="prediction-card border-medium"><div class="pred-label">10年後 (年3%)</div><div class="pred-price">{round(p_10y[1]):,}</div><div class="pred-diff">現在比 {get_diff_text(p_10y[1], price_now)}</div></div>', unsafe_allow_html=True)
    with col10_3: st.markdown(f'<div class="prediction-card border-heavy"><div class="pred-label">10年後 (年5%)</div><div class="pred-price">{round(p_10y[2]):,}</div><div class="pred-diff">現在比 {get_diff_text(p_10y[2], price_now)}</div></div>', unsafe_allow_html=True)
